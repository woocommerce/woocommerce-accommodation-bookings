...
**Never trust data that flows through hooks.** Keep hook callback parameters untyped and validate or coerce the value before passing it to strictly typed code, since any callback can receive a value another one produced. And when firing a filter, validate the final return value before using it, since any callback in the chain can return the wrong thing: `WC_Product_Accommodation_Booking::get_check_times()` returns whatever the last `woocommerce_accommodation_booking_get_check_times` callback produced, and that value flows unchecked into date-picker output and order item meta as if it were still an `H:i` time string. The same distrust applies on the consumer side - the `woocommerce_bookings_*` filters this plugin hooks can hand its callbacks a value some other callback reshaped, so type-check there too instead of assuming Bookings' documented types.
