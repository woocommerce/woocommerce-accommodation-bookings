 ...
 */
const fs = require("fs");
const { admin, customer, baseUrl } = require("./config");
...
module.exports = async (config) => {
  ...
  for (let i = 0; i < nRetries; i++) {
    try {
      ...
      await adminPage.locator("#key_description").fill("Key for API access");
      await adminPage.locator("#key_permissions").selectOption("read_write");
