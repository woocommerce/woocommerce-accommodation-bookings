...
class WC_Accommodation_Booking_Admin_Panels {
	...
	public function save_product_data( $post_id ) {
		...
		// Rates
		$pricing = array();
		$original_base_cost = NumberUtil::abs( get_post_meta( $post_id, '_wc_booking_base_cost', true ) );
		...
		for ( $i = 0; $i < $row_size; $i ++ ) {
			...
			$new_cost = NumberUtil::abs( wc_clean( $_POST[ 'wc_accommodation_booking_pricing_block_cost' ][ $i ] ) );
			$pricing[ $i ]['base_modifier'] = $pricing[$i]['modifier'] = $new_cost > $original_base_cost ? 'plus' : 'minus';
			$pricing[ $i ]['cost'] = NumberUtil::abs( $new_cost - $original_base_cost );
